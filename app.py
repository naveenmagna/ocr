...
# st.title("Image Text Extraction and CSV Export")
...
# # Create a Streamlit sidebar for "dotted" images
# st.sidebar.title('Dotted Images OCR')

# # Upload multiple dotted images
# dotted_uploaded_files = st.sidebar.file_uploader("Upload dotted image files", type=["jpg", "png"], accept_multiple_files=True)

# # Create an empty DataFrame to store the text data from dotted images
# dotted_all_data = pd.DataFrame()
...
#         # Apply Gaussian blur to denoise and smooth the image
#         blurred_image = cv2.GaussianBlur(gray_image, (5, 5), 0)
        
#         # Apply dilation to make text bolder
#         kernel_dilation = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))
#         dilated_image = cv2.dilate(blurred_image, kernel_dilation, iterations=1)
        
#         # Apply erosion to remove noise
#         kernel_erosion = cv2.getStructuringElement(cv2.MORPH_RECT, (1, 1))
#         eroded_image = cv2.erode(blurred_image, kernel_erosion, iterations=1)
        
#         # Adjust the thresholding method to improve text separation
#         _, threshold_image = cv2.threshold(eroded_image, 220, 255, cv2.THRESH_BINARY  )


#         # # Apply the Sobel filter for edge detection
#         # sobel_x = cv2.Sobel(image, cv2.CV_64F, 1, 0, ksize=3)  # Sobel filter in the X direction
#         # sobel_y = cv2.Sobel(image, cv2.CV_64F, 0, 1, ksize=3)  # Sobel filter in the Y direction
        
#         # # Calculate the magnitude of the gradient
#         # gradient_magnitude = np.sqrt(sobel_x**2 + sobel_y**2)
        
#         # # Convert the gradient magnitude to 8-bit format for visualization
#         # gradient_magnitude = cv2.convertScaleAbs(threshold_image)
...
#         # Append the data to the dotted_all_data DataFrame
#         dotted_all_data = pd.concat([dotted_all_data, df])
...
#     # Display the processed data for all dotted images (if any)
#     st.sidebar.write("Processed Data for Dotted Images:")
#     st.sidebar.dataframe(dotted_all_data)

# st.sidebar.info('Upload dotted images here separately.')
...
#         # Perform OCR on the edge-detected image
#         text_data = pytesseract.image_to_string(gradient_magnitude, config=custom_config)
#         text_data = text_data.replace("  ", " ")
#         text_data = text_data.replace(".  ", " ")
#         text_data = text_data.replace(" .", ".")
...
#         edges = cv2.Canny(image, threshold1=255, threshold2=255)  # Adjust the thresholds as needed

#         custom_config = r'--oem 3 --psm 6 -c tessedit_char_whitelist="0123456789 ." -c classify_bln_numeric_mode=1 -c tessedit_char_blacklist=ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'

#         # Perform OCR on the edge-detected image
#         text_data = pytesseract.image_to_string(edges, config=custom_config)
#         text_data = text_data.replace("  ", " ")
#         text_data = text_data.replace(". ", ".")
#         text_data = text_data.replace(" .", ".")
